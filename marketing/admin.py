from django.contrib import admin
from django.utils.html import format_html
from .models import Coupon, HomeSlider, DealOfTheDay, SellingNote
from core.admin_mixins import ChangeHistoryMixin

@admin.register(Coupon)
class CouponAdmin(ChangeHistoryMixin, admin.ModelAdmin):
    list_display = ['code', 'discount_percent', 'valid_from', 'valid_to', 'active']
    list_filter = ['active', 'valid_to']
    search_fields = ['code']

@admin.register(HomeSlider)
class HomeSliderAdmin(ChangeHistoryMixin, admin.ModelAdmin):
    list_display = ['title', 'display_image', 'sort_order', 'is_active', 'link_url']
    list_editable = ['sort_order', 'is_active']
    search_fields = ['title']

    def display_image(self, obj):
        if obj.image:
            return format_html('<img src="{}" style="width: 120px; height: 50px; object-fit: cover; border-radius: 4px;" />', obj.image.url)
        return "-"
    display_image.short_description = "Preview"

@admin.register(DealOfTheDay)
class DealOfTheDayAdmin(ChangeHistoryMixin, admin.ModelAdmin):
    list_display = ['title', 'discount_percentage', 'end_date', 'is_active', 'created_at']
    list_editable = ['is_active', 'end_date']
    list_filter = ['is_active', 'end_date']
    search_fields = ['title', 'subtitle']

@admin.register(SellingNote)
class SellingNoteAdmin(ChangeHistoryMixin, admin.ModelAdmin):
    list_display = ['title', 'subtitle', 'icon', 'sort_order', 'is_active']
    list_editable = ['sort_order', 'is_active']
    search_fields = ['title', 'subtitle']
